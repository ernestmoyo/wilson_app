...
router.get('/overdue-notifications', (_req: Request, res: Response) => {
  try {
  ...
  try {
    ...
    try {
      db.prepare('INSERT INTO audit_log (entity_type, entity_id, action, user_id, details) VALUES (?,?,?,?,?)')
        .run('certificate', newId, 'created', inspector_id, JSON.stringify({ certificate_type, client_id }));
    } catch (_) {}
...
router.put('/:id', (req: Request, res: Response) => {
  try {
    ...
    try {
      db.prepare('INSERT INTO audit_log (entity_type, entity_id, action, user_id, details) VALUES (?,?,?,?,?)')
        .run('certificate', req.params.id, status === 'refused' ? 'refused' : 'updated', cert.inspector_id, JSON.stringify({ status, prev_status: cert.status }));
    } catch (_) {}
...
router.get('/:id/refusal-form', (req: Request, res: Response) => {
  try {
