...
router.post('/', (req: Request, res: Response) => {
  try {
    ...
    db.prepare(`
      ...
      VALUES ('enquiry', ?, 'created', ?)
    `).run(enquiryId, JSON.stringify({ contact_name, enquiry_type }));
...
router.put('/:id/convert', (req: Request, res: Response) => {
  try {
    ...
    try {
      ...
      db.prepare(`
        INSERT INTO audit_log (entity_type, entity_id, action, details)
        VALUES ('enquiry', ?, 'converted', ?)
      `).run(req.params.id, JSON.stringify({ converted_assessment_id: assessmentId }));
...
router.delete('/:id', (req: Request, res: Response) => {
  try {
    ...
    db.prepare(`
      INSERT INTO audit_log (entity_type, entity_id, action, details)
      VALUES ('enquiry', ?, 'deleted', ?)
    `).run(req.params.id, JSON.stringify(enquiry));
